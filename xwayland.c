...
xwayland_view_from_view(struct nedm_view *view) {
	return (struct nedm_xwayland_view *)view;
}

static const struct nedm_xwayland_view *
xwayland_view_from_const_view(const struct nedm_view *view) {
	return (const struct nedm_xwayland_view *)view;
}
...
activate(struct nedm_view *view, bool activate) {
	struct nedm_xwayland_view *xwayland_view = xwayland_view_from_view(view);
...
handle_xwayland_surface_request_fullscreen(struct wl_listener *listener,
	...
	struct nedm_xwayland_view *xwayland_view =
...
handle_xwayland_surface_unmap(struct wl_listener *listener,
	...
	struct nedm_view *view = &xwayland_view->view;
...
static const struct nedm_view_impl xwayland_view_impl = {
    .get_title = get_title,
    .get_pid = get_pid,
...
handle_xwayland_surface_new(struct wl_listener *listener, void *data) {
	...
	struct nedm_xwayland_view *xwayland_view =
	    calloc(1, sizeof(struct nedm_xwayland_view));
	if(!xwayland_view) {
	...
	view_init(&xwayland_view->view, NEDM_XWAYLAND_VIEW, &xwayland_view_impl,
	          server);
